...
class Summarizer:
    ...
    def __init__(self, api_key: str = None, base_url: str = None, model: str = None):
        ...
        self._timeout = float(os.getenv("OPENAI_SUMMARY_TIMEOUT", "120"))

        if not effective_key:
            logger.warning("未设置OPENAI_API_KEY环境变量，将无法使用摘要功能")

        if effective_key:
            kwargs = {"api_key": effective_key, "timeout": self._timeout}
            if effective_url:
        ...
        env_model = os.getenv("OPENAI_SUMMARY_MODEL") or os.getenv("OPENAI_MODEL")
    ...
    async def _format_long_transcript_in_chunks(
        ...
        for i, c in enumerate(final_chunks):
            if progress_callback:
                await progress_callback(i + 1, len(final_chunks))
