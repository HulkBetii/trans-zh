...
class Transcriber:
    ...
    def __init__(self, model_size: Optional[str] = None):
        ...
        """
        self.model_size = (model_size or os.getenv("WHISPER_MODEL_SIZE") or "base").strip()
        self.requested_device = (os.getenv("WHISPER_DEVICE") or "cpu").strip().lower()
        self.requested_compute_type = (os.getenv("WHISPER_COMPUTE_TYPE") or "").strip().lower()
        self.active_device = ""
    ...
    def _resolve_runtime(self) -> tuple[str, str]:
        device = self.requested_device if self.requested_device in {"cpu", "cuda", "auto"} else "cpu"
        cuda_available = ctranslate2.get_cuda_device_count() > 0
        if device == "auto":
            device = "cuda" if cuda_available else "cpu"
        elif device == "cuda" and not cuda_available:
            logger.warning("WHISPER_DEVICE=cuda requested but CUDA is unavailable; falling back to CPU")
            device = "cpu"

        compute_type = self.requested_compute_type or ("float16" if device == "cuda" else "int8")
    ...
    def runtime_info(self) -> dict:
        ...
        return {
            ...
            "device": self.active_device or device,
            "compute_type": self.active_compute_type or compute_type,
        ...
        if self.model is None:
            ...
            try:
                ...
                try:
                    ...
                    if device != "cuda":
                        raise
                    logger.warning("CUDA Whisper initialization failed; falling back to CPU/int8: %s", exc)
        ...
        if transcript_text and "**Detected Language:**" in transcript_text:
            ...
            for line in lines:
                if "**Detected Language:**" in line:
                    lang = line.split(":")[-1].strip().strip("*").strip()
