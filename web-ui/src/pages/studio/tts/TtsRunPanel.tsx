...
export function TtsOutput({ output, stale }: { output: NonNullable<TtsWorkspace["output"]>; stale: boolean }) {
  ...
  return (
    <section className={`tts-output panel-sheet ${stale || output.state !== "current" ? "stale" : ""}`}>
      <div className="tts-output-heading"><div><span className="eyebrow">Timeline deliverable</span><h2>Audio lồng tiếng {output.language === "en" ? "tiếng Anh" : "tiếng Việt"}</h2><p>{stale || output.state !== "current" ? "Output không còn khớp với văn bản hiện tại; cần tạo lại." : "MP3 đã ráp theo mốc thời gian ASR."}</p></div>{available && <a className="secondary-button compact" href={href} download><AudioLines size={15} />Tải MP3</a>}</div>
      {!available ? <p className="tts-danger">{output.state === "missing" ? "File không còn trên đĩa." : "Không có đường dẫn tải an toàn cho file này."}</p> : <audio className="tts-timeline-audio" src={href} controls preload="metadata" aria-label={`Nghe audio lồng tiếng ${output.language === "en" ? "tiếng Anh" : "tiếng Việt"}`} />}
