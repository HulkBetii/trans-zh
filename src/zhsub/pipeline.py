...
def run_job(
    ...
    for stage in todo:
        ...
        try:
            _run_stage(stage, req, cfg, work_dir, job_id)
...
def _run_stage(stage: str, req: RunRequest, cfg: Config, work_dir: Path, job_id: str) -> None:
    ...
    if stage == "ingest":
        ...
        s1_asr.run(work_dir, cfg, force=force_here)
    elif stage == "segment":
        s2_segment.run(work_dir, cfg, force=force_here)
    elif stage == "glossary":
        s3_glossary.run(work_dir, cfg, force=force_here)
    elif stage == "translate":
        s4_translate.run(work_dir, cfg, req.targets, force=force_here)
    elif stage == "render":
        s5_render.run(
